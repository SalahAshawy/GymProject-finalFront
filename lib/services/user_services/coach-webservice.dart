...
class CoachWebService {
  ...
  static void addCoach({
    ...
    DioHelper.postData(url: 'api/coaches/store', data: formData).then((value) {
  ...
  static void updateCoach({
    ...
    DioHelper.postData(url: 'api/coaches/update/$id', data: formData)
        .then((value) {
      var jsonData = value.data;
      if (jsonData['status'] == true) {
  ...
  static void assignMember({
    ...
    http
        .post(
            Uri.parse(
                '${Constants.defaultUrl}/api/coaches/assignMember/${coach.id}'),
            headers: <String, String>{
              'Content-Type': 'application/json',
              'Accept': 'application/json',
              'Authorization': 'Bearer $token'
  ...
  Future<Tuple2<List<PrivateSession>, List<Classes>, List<Event>>>
    ...
    final response =
        await http.get(Uri.parse('$local/api/coaches/schedule/self'), headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
      'Authorization': 'Bearer $token'
    });
