...
class NutritionistWebservice {
  ...
  static void addNutritionist({
    ...
    DioHelper.postData(url: 'api/nutritionists/store', data: formData)
        .then((value) {
      var jsonData = value.data;
      if (jsonData['status'] == true) {
        Nutritionist nutritionists =
            Nutritionist.fromJsonAdd(jsonData['Nutritionist']);
  ...
  static void updateNutritionist({
    ...
    DioHelper.postData(url: 'api/nutritionists/update/$id', data: formData)
        .then((value) {
      var jsonData = value.data;
      if (jsonData['status'] == true) {
        nutritionistsUsers[nutritionistsIndex]
            .update(jsonData['Nutritionist'][0]);
  ...
  static void assignMember({
    ...
    http
        .post(
            Uri.parse(
                '${Constants.defaultUrl}/api/nutritionists/assignMember/${nutritionist.id}'),
            headers: <String, String>{
              'Content-Type': 'application/json',
              'Accept': 'application/json',
              'Authorization': 'Bearer $token'
