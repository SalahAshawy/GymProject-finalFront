...
class AnnouncementsServices {
    ...
    await http.get(Uri.parse('$local/api/announcements'), headers: {
      ...
      jsonData['announcments'].forEach((item) {
        announcementsList.add(
            AnnouncementViewModel(announcement: Announcement.fromJson(item)));
      });
    }).catchError((error) {
  ...
  //add announcement
  static void addAnnouncement(String title, String description, String date,
      {required AdminCubit adminCubit, required BuildContext context}) {
    ...
    http
      ...
      announcementsList.add(AnnouncementViewModel(
          announcement: Announcement.fromJson(jsonData['newAnnouncement'])));
      ...
      if (jsonData['msg'] == 'Announcement updated correctly') {
        for (int i = 0; i < announcementsList.length; i++) {
          if (announcementsList.elementAt(i).id == id) {
            announcementsList[i] = AnnouncementViewModel(
